...
class BaseMigration extends Migration
{
	...
	public function addUnsigned($type, $key)
	{
		$types = [
			"boolean",
			"dateTime",
	...
	public function addForeignMany($key)
	{
	...
	public function addForeign($key, $id, $fk)
	{
		$this->addUnsigned("integer", $key);
		$this->getTable()->foreign($key)->references($id)->on($fk);//>onDelete('cascade')->onUpdate('cascade');
	...
	public function addSmallInteger($key)
	{
		return $this->addNullable("smallInteger", $key);
	}
	...
	{
		return $this->addNullable("float", $key);
	}
